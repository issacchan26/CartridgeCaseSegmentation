...
class Params(object):
    def __init__(self, hf_dataset_identifier, pretrained_model_name, prediction_save_path, infer_folder):
...
def get_prediction(infer_folder, model_name, prediction_save_path):
    with torch.no_grad():
        processor = SegformerImageProcessor(size = {"height": 256, "width": 256})
        model = SegformerForSemanticSegmentation.from_pretrained(model_name)
        model.eval()

        for k in os.listdir(infer_folder):
            fname = k.split('.')[0]
            image = Image.open(os.path.join(infer_folder, k)).convert("RGB")
            ...
            cv2.imwrite(prediction_save_path + fname + '_result.png', pred_img)
    return
...
if __name__ == '__main__':
    ...
    get_prediction(args.infer_folder, args.pretrained_model_name, args.prediction_save_path)
